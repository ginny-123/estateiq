...
function model(base:typeof defaults, vacancy=base.vacancy, maintenance=base.maintenance) {
  ...
  const combinedIrr = cashInvested > 0 && combinedExitValue > 0 ? (Math.pow(combinedExitValue / cashInvested, 1 / base.years) - 1) * 100 : 0;
...
export default function Home(){
  ...
  return <main className={`mobile-tab-${mobileTab}`}>
    ...
    <div className="sticky-summary"><b>{roiLabel}: {percent(c.headlineRoi)}</b><span>Cap {percent(c.capRate)}</span><span>CoC {percent(c.coc)}</span><span>IRR {percent(c.combinedIrr)}</span></div>
    <section className="dashboard">
      ...
      <div className="results-area">
        ...
        <div className="scenario-row mobile-pane mobile-projection">{scenarios.map(s=><div key={s.name}><b>{s.name}</b><span>CoC {percent(s.coc)}</span><span>DSCR {s.dscr.toFixed(2)}x</span><span>5-yr IRR {percent(s.combinedIrr)}</span></div>)}</div>
        ...
        <section className="panel market-section mobile-pane mobile-exit"><h2>Property + reserve IRR</h2><div className="market-grid"><Card label="Initial investment" value={money(c.cashInvested)}/><Card label="Property net equity at exit" value={money(c.propertyExitEquity)}/><Card label="Cash + tax reserve (held, not invested)" value={money(c.reserve)}/><Card label="Combined exit value" value={money(c.combinedExitValue)}/><Card label="Annualized IRR / CAGR" value={percent(c.combinedIrr)} accent/></div><p>Operating cash flow and each year's net tax effect are held at face value — no market reinvestment is assumed. This is why Combined exit value − Initial investment ({money(c.combinedExitValue-c.cashInvested)}) always exactly equals Total profit ({money(c.totalProfit)}) above, and why this IRR matches {roiLabel}.</p><div className="hypothetical-callout"><b>Optional comparison — not included in any total above</b><span>If instead you separately invested that same cash flow + tax reserve in the market at {percent(v.stockReturn)}/yr: hypothetical ending value {money(c.hypotheticalMarketExitValue)}, hypothetical IRR {percent(c.hypotheticalMarketIrr)}. Does not account for taxes on stock gains or dividends.</span></div></section>
