...
function model(base:typeof defaults, vacancy=base.vacancy, maintenance=base.maintenance) {
  ...
  let balance = loan, sidecar = 0, suspended = 0, cumulativeCash = 0, cumulativeUsableTax = 0, totalLossPool = 0;
  ...
  for (let m=0; m<months; m++) {
    ...
    if ((m+1)%12===0 || m===months-1) {
      ...
      // STEP 1: total loss pool for the year — mortgage interest + depreciation minus net rental
      // income. This is computed ONCE and is 100% independent of Tax Profile. Tax Profile only
      // ever changes how this fixed pool is split between "usable now" and "suspended until sale".
      const generatedLoss = Math.max(0, interestAndDepreciation - operatingBeforeInterest);
      totalLossPool += generatedLoss;
      // STEP 2: allocate the SAME pool across the three IRC §469 treatments — split only, no re-derivation.
      let usableLoss = 0;
      ...
      // Reinvest this year's usable tax benefit into the S&P sidecar at the same point it's earned,
      // so it compounds at the reinvestment rate for the remainder of the hold period (same treatment
      // as any other cash flow). Suspended-loss release at sale is NOT added here — see sale section.
      sidecar += currentTaxBenefit;
      yearBucket.taxBenefit = currentTaxBenefit; yearBucket.suspended = suspended;
      yearBucket.generatedLoss = generatedLoss; yearBucket.usableLoss = usableLoss;
  ...
  return { monthly, yearly, first, loan, payment, balance, cashInvested, capRate, coc, dscr, cumulativeCash, cumulativeUsableTax, suspended, suspendedReleaseBenefit, totalLossPool, sidecar, salePrice, sellingCosts, adjustedBasis, accumulatedDepreciation, totalGain, recaptureGain, capitalGain, fullRecaptureTax, fullCapitalGainTax, fullSaleTax, preTaxEquity, taxableNetSale, selectedNetSale, totalProfit, headlineRoi, boot, bootRatio, bootTax, netBootCash, deferredTax, carryoverBasis, replacementDepreciableBasis, replacementAnnualDepreciation, propertyExitEquity, combinedExitValue, marketIrr };
}
