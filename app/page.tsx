...
export default function Home(){
  ...
  return <main className={`mobile-tab-${mobileTab}`}>
    ...
    <div className="sticky-summary"><b>{roiLabel}: {percent(c.headlineRoi)}</b><span>Cap {percent(c.capRate)}</span><span>CoC {percent(c.coc)}</span><span>After-tax IRR {percent(c.afterTaxIrr)}</span><span>Pre-tax IRR {percent(c.pretaxIrr)}</span></div>
